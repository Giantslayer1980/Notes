...
// 关于PreferenceKey的使用
// 当某个场景下,父view需要获取子view的某些信息,就可以考虑使用PreferenceKey这个技术,
// 它最大的优点是可以让子view封装任何信息.
// 但这可能存在一些潜在的问题,如果父view的布局改变了,影响到了子view的布局,子view的布局又影响了父view的布局,
// 这种情况可能会出现死循环。（是不是在构建父view和子view时,尽量不让两者的布局产生改变,是不是就能解决这个问题吗？本子中父view不会改变布局,子view也不会改变布局)

// PreferenceKey怎么在本例中使用的大致思路:
// 首先,要给每个月份元素添加实现PreferenceKey协议的值,每个月份元素都是由MonthView结构来实现的,该MonthView返回的view中,
// 在.background()修饰符中包含了MyPreferenceViewSetter方法,该方法返回的Rectangle图形中又用到了.preference修饰器,该修饰器的key/value组合以供后续父view调取数据;
// 其次,VStack中包含的所有月份元素,又会因为.onPreferenceChange修饰器中的代码的执行而将所有的月份的要素全部归于rects变量,
// 再次,ZStack中的RoundedRectangle()生成,又因@State修饰的rects而生成初始值,即实时形成最初的框,
// 最后,因为@State修饰的activeIdx的变化,而使RoundedRectangle()这个view的大小和位置实时去变化。
...
struct PreferenceKeyExample: View {
    ...
    var body: some View {
        ...
        ZStack(alignment: .topLeading) {
            ...
            // ZStack通过.onPreferenceChange获取了全部的preferences,然后根据包裹中的数据给self.rects赋值
    ...
    var body: some View {
        GeometryReader { geometry in
            Rectangle()
                ...
                // key只需要实现PreferenceKey协议即可,该协议要求实现一个静态变量defaultValue和静态函数reduce
                // value放入封装好的MyTextPreferenceData这个结构体
                // 当父view想要获取子view信息的时候,它就会遍历view中的reduce,然后把所有的包裹合并成一个数组
                // 这里用到了GeometryReader的.frame(in: .named())命名空间,不同于用.global和.local,应该是更好定位。
